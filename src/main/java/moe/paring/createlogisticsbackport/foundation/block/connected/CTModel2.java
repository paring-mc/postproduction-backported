...
public class CTModel2 extends BakedModelWrapperWithData {
	...
	protected CTData createCTData(BlockAndTintGetter world, BlockPos pos, BlockState state) {
		...
		for (Direction face : Iterate.directions) {
			...
			if (!behaviour.buildContextForOccludedDirections()
				...
				continue;
			CTType2 dataType = behaviour.getDataType(world, pos, state, face);
			if (dataType == null)
				continue;
	...
	public List<BakedQuad> getQuads(BlockState state, Direction side, RandomSource rand, ModelData extraData, RenderType renderType) {
		...
		for (int i = 0; i < quads.size(); i++) {
			...
			CTSpriteShiftEntry2 spriteShift = behaviour.getShift(state, rand, quad.getDirection(), quad.getSprite());
			if (spriteShift == null)
				continue;
