...
import moe.paring.createlogisticsbackport.content.logistics.box.PackageStyles.PackageStyle;
import moe.paring.createlogisticsbackport.content.logistics.stockTicker.PackageOrderWithCrafts;
import moe.paring.createlogisticsbackport.foundation.data.Glob;
...
public class PackageItem extends Item {
  ...
  public static boolean isFinalLink(ItemStack box) {
	...
	public InteractionResultHolder<ItemStack> open(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		ExtraSoundEvents.PACKAGE_POP.playOnServer(worldIn, playerIn.blockPosition());
	...
	public InteractionResult useOn(UseOnContext context) {
		...
		if (!world.getEntities(ExtraEntityTypes.PACKAGE.get(), scanBB, e -> true)
			.isEmpty())
