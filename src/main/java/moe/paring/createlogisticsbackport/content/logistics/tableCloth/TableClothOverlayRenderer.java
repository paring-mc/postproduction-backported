...
public class TableClothOverlayRenderer {
	...
	public static void tick() {
		...
		if (mouseOver.getType() != Type.ENTITY) {
			...
			BlueprintOverlayRenderer2.displayClothShop(dcbe, alreadyPurchased, list);
			return;
		}

		EntityHitResult entityRay = (EntityHitResult) mouseOver;
		if (!ExtraItems.SHOPPING_LIST.isIn(heldItem))
			return;
		...
		BlueprintOverlayRenderer2.displayShoppingList(list.bakeEntries(mc.level, null));
	}
