...
public class LogisticallyLinkedBehaviour extends BlockEntityBehaviour {
	...
	public void unload() {
		if (loadedGlobally && global && getWorld() != null)
			CreateLogisticsBackport.LOGISTICS.linkInvalidated(freqId, getGlobalPos());
		super.unload();
		remove(this);
	}
	...
	public void initialize() {
		...
		if (!loadedGlobally && global) {
			loadedGlobally = true;
			CreateLogisticsBackport.LOGISTICS.linkLoaded(freqId, getGlobalPos());
		}
		...
		if (!addedGlobally && global) {
			...
			if (blockEntity instanceof PackagerLinkBlockEntity plbe)
				CreateLogisticsBackport.LOGISTICS.linkAdded(freqId, getGlobalPos(), plbe.placedBy);
		}
	...
	public void destroy() {
		...
		if (addedGlobally && global && getWorld() != null)
			CreateLogisticsBackport.LOGISTICS.linkRemoved(freqId, getGlobalPos());
	}
	...
	public boolean mayInteract(Player player) {
		return CreateLogisticsBackport.LOGISTICS.mayInteract(freqId, player);
	}

	public boolean mayInteractMessage(Player player) {
		boolean mayInteract = CreateLogisticsBackport.LOGISTICS.mayInteract(freqId, player);
		if (!mayInteract)
	...
	public boolean mayAdministrate(Player player) {
		return CreateLogisticsBackport.LOGISTICS.mayAdministrate(freqId, player);
	}
