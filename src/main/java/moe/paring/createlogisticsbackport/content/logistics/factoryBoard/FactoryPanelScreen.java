...
import static moe.paring.createlogisticsbackport.registry.ExtraGuiTextures.*;
...
public class FactoryPanelScreen extends AbstractSimiScreen {
	...
	protected void init() {
		...
		confirmButton = new NewIconButton(x + sizeX - 33, y + sizeY - 25, AllIcons.I_CONFIRM);
		...
		deleteButton = new NewIconButton(x + sizeX - 55, y + sizeY - 25, AllIcons.I_TRASH);
		deleteButton.withCallback(() -> {
			sendReset = true;
			minecraft.setScreen(null);
		});
		deleteButton.setToolTip(Lang.translate("gui.factory_panel.reset")
			.component());
		addRenderableWidget(deleteButton);

		promiseExpiration = new ScrollInput(x + 97, y + windowHeight - 24, 28, 16).withRange(-1, 31)
			.titled(Lang.translate("gui.factory_panel.promises_expire_title")
				.component());
		promiseExpiration.setState(behaviour.promiseClearingInterval);
		addRenderableWidget(promiseExpiration);

		newInputButton = new NewIconButton(x + 31, y + 47, AllIcons.I_ADD);
		newInputButton.withCallback(() -> {
			FactoryPanelConnectionHandler.startConnection(behaviour);
			minecraft.setScreen(null);
		});
		newInputButton.setToolTip(Lang.translate("gui.factory_panel.connect_input")
			.component());

		relocateButton = new NewIconButton(x + 31, y + 67, ExtraIcons.I_MOVE_GAUGE);
		relocateButton.withCallback(() -> {
			FactoryPanelConnectionHandler.startRelocating(behaviour);
			minecraft.setScreen(null);
		});
		relocateButton.setToolTip(Lang.translate("gui.factory_panel.relocate")
			.component());
		...
		if (availableCraftingRecipe != null) {
			activateCraftingButton = new NewIconButton(x + 31, y + 27, AllIcons.I_3x3);
			...
			activateCraftingButton.setToolTip(Lang.translate("gui.factory_panel.activate_crafting")
				.component());
	...
	protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		// BG
		ExtraGuiTextures bg = restocker ? FACTORY_GAUGE_RESTOCK : FACTORY_GAUGE_RECIPE;
		if (restocker)
			...
			if (inputConfig.isEmpty()) {
				...
				if (!restocker && mouseY > inputY && mouseY < inputY + 60 && mouseX > inputX && mouseX < inputX + 60)
					graphics.renderComponentTooltip(font,
						List.of(Lang.translate("gui.factory_panel.unconfigured_input")
							.color(ScrollInput.HEADER_RGB)
							.component(),
							Lang.translate("gui.factory_panel.unconfigured_input_tip")
								.style(ChatFormatting.GRAY)
								.component(),
							Lang.translate("gui.factory_panel.unconfigured_input_tip_1")
								.style(ChatFormatting.GRAY)
								.component()),
						mouseX, mouseY);
			}
		...
		if (!restocker) {
			...
			if (mouseX >= outputX - 1 && mouseX < outputX - 1 + 18 && mouseY >= outputY - 1
				&& mouseY < outputY - 1 + 18) {
				MutableComponent c1 = Lang
					.translate("gui.factory_panel.expected_output", Lang.itemName(outputConfig.stack)
						.add(Lang.text(" x" + outputConfig.count))
						.string())
					.color(ScrollInput.HEADER_RGB)
					.component();
				MutableComponent c2 = Lang.translate("gui.factory_panel.expected_output_tip")
					.style(ChatFormatting.GRAY)
					.component();
				MutableComponent c3 = Lang.translate("gui.factory_panel.expected_output_tip_1")
					.style(ChatFormatting.GRAY)
					.component();
				MutableComponent c4 = Lang.translate("gui.factory_panel.expected_output_tip_2")
		...
		GuiGameElement.of(ExtraBlocks.FACTORY_GAUGE.asStack())
			.scale(4)
			.at(0, 0, -200)
		...
		if (!behaviour.targetedByLinks.isEmpty()) {
			...
			if (mouseX >= itemX && mouseX < itemX + 16 && mouseY >= itemY && mouseY < itemY + 16) {
				List<Component> linkTip = List.of(Lang.translate("gui.factory_panel.has_link_connections")
					.color(ScrollInput.HEADER_RGB)
					.component(),
					Lang.translate("gui.factory_panel.left_click_disconnect")
						.style(ChatFormatting.DARK_GRAY)
						.style(ChatFormatting.ITALIC)
						.component());
		...
		graphics.drawString(font, Lang.text(state == -1 ? " /" : state == 0 ? "30s" : state + "m")
		...
		if (mouseX >= itemX && mouseX < itemX + 16 && mouseY >= itemY && mouseY < itemY + 16) {
			...
			if (promised == 0) {
				promiseTip = List.of(Lang.translate("gui.factory_panel.no_open_promises")
					.color(ScrollInput.HEADER_RGB)
					.component(),
					Lang
					...
					Lang
						...
						.style(ChatFormatting.GRAY)
						.component(),
					Lang.translate("gui.factory_panel.promise_prevents_oversending")
						.style(ChatFormatting.GRAY)
						.component());
			} else {
				promiseTip = List.of(Lang.translate("gui.factory_panel.promised_items")
					.color(ScrollInput.HEADER_RGB)
					.component(),
					Lang.text(behaviour.getFilter()
						.getHoverName()
						.getString() + " x" + promised)
						.component(),
					Lang.translate("gui.factory_panel.left_click_reset")
						.style(ChatFormatting.DARK_GRAY)
						.style(ChatFormatting.ITALIC)
						.component());
			}
	...
	private void renderInputItem(GuiGraphics graphics, int slot, BigItemStack itemStack, int mouseX, int mouseY) {
		...
		if (craftingActive) {
			graphics.renderComponentTooltip(font, List.of(Lang.translate("gui.factory_panel.crafting_input")
				.color(ScrollInput.HEADER_RGB)
				.component(),
				Lang.translate("gui.factory_panel.crafting_input_tip")
					.style(ChatFormatting.GRAY)
					.component(),
				Lang.translate("gui.factory_panel.crafting_input_tip_1")
					.style(ChatFormatting.GRAY)
					.component()),
				mouseX, mouseY);
			return;
		...
		if (itemStack.stack.isEmpty()) {
			graphics.renderComponentTooltip(font, List.of(Lang.translate("gui.factory_panel.empty_panel")
				.color(ScrollInput.HEADER_RGB)
				.component(),
				Lang.translate("gui.factory_panel.left_click_disconnect")
					.style(ChatFormatting.DARK_GRAY)
		...
		if (restocker) {
			graphics.renderComponentTooltip(font,
				List.of(Lang.translate("gui.factory_panel.sending_item", Lang.itemName(itemStack.stack)
					.string())
					.color(ScrollInput.HEADER_RGB)
					.component(),
					Lang.translate("gui.factory_panel.sending_item_tip")
						.style(ChatFormatting.GRAY)
						.component(),
					Lang.translate("gui.factory_panel.sending_item_tip_1")
						.style(ChatFormatting.GRAY)
						.component()),
				mouseX, mouseY);
			return;
		...
		graphics.renderComponentTooltip(font,
			List.of(Lang.translate("gui.factory_panel.sending_item", Lang.itemName(itemStack.stack)
				.add(Lang.text(" x" + itemStack.count))
				.string())
				.color(ScrollInput.HEADER_RGB)
				.component(),
	...
	private void showAddressBoxTooltip(GuiGraphics graphics, int mouseX, int mouseY) {
		if (addressBox.getValue()
			...
			if (restocker) {
				graphics.renderComponentTooltip(font,
					List.of(Lang.translate("gui.factory_panel.restocker_address")
						.color(ScrollInput.HEADER_RGB)
						.component(),
						Lang.translate("gui.factory_panel.restocker_address_tip")
							.style(ChatFormatting.GRAY)
							.component(),
						Lang.translate("gui.factory_panel.restocker_address_tip_1")
							.style(ChatFormatting.GRAY)
							.component(),
				...
				graphics.renderComponentTooltip(font, List.of(Lang.translate("gui.factory_panel.recipe_address")
					.color(ScrollInput.HEADER_RGB)
					.component(),
					Lang.translate("gui.factory_panel.recipe_address_tip")
						.style(ChatFormatting.GRAY)
						.component(),
					Lang.translate("gui.factory_panel.recipe_address_tip_1")
						.style(ChatFormatting.GRAY)
						.component(),
					Lang.translate("gui.schedule.lmb_edit")
						.style(ChatFormatting.DARK_GRAY)
