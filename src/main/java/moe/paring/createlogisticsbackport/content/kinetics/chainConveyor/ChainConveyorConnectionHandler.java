...
import moe.paring.createlogisticsbackport.polyfill.BlueprintOverlayRendererExtras;
import moe.paring.createlogisticsbackport.registry.ExtraPackets;
...
public class ChainConveyorConnectionHandler {
	...
	public static boolean onRightClick() {
		...
		if (!mc.player.isShiftKeyDown() && !missed)
			return false;
		firstPos = null;
		Lang.translate("chain_conveyor.selection_cleared")
	...
	public static void onItemUsedOnBlock(PlayerInteractEvent.RightClickBlock event) {
		...
		if (level.getBlockEntity(pos) instanceof ChainConveyorBlockEntity ccbe
			&& ccbe.connections.size() >= ExtraConfigs.server().kinetics.maxChainConveyorConnections.get()) {
			Lang.translate("chain_conveyor.cannot_add_more_connections")
	...
	public static void clientTick() {
		...
		if (pos.equals(firstPos)) {
			highlightConveyor(firstPos, 0xFFFFFF, "chain_connect");
			Lang.translate("chain_conveyor.select_second")
				.sendStatus(player);
			return;
		}
		...
		if (success)
			Lang.translate("chain_conveyor.valid_connection")
		...
		CreateClient.OUTLINER.showLine("chain_connect_line", from.add(normal), to.add(normal))
			.lineWidth(1 / 16f)
			.colored(color);
		CreateClient.OUTLINER.showLine("chain_connect_line_1", from.subtract(normal), to.subtract(normal))
			.lineWidth(1 / 16f)
			.colored(color);
		...
		for (int y : Iterate.zeroAndOne) {
			...
			for (int i = 0; i < 8; i++) {
				Vec3 v = VecHelper.rotate(new Vec3(0, .125 + y * .75, 1.25), 22.5 + i * 45, Axis.Y)
					.add(Vec3.atBottomCenterOf(pos));
				CreateClient.OUTLINER.showLine(key + y + i, prevV, v)
					.lineWidth(1 / 16f)
					.colored(color);
				prevV = v;
			}
	...
	public static boolean validateAndConnect(LevelAccessor level, BlockPos pos, Player player, ItemStack chain,
		...
		if (pos.equals(firstPos))
			return false;
		if (!pos.closerThan(firstPos, ExtraConfigs.server().kinetics.maxChainConveyorLength.get()))
		...
		ChainConveyorBlock chainConveyorBlock = ExtraBlocks.CHAIN_CONVEYOR.get();
		...
		if (targetLift.connections.size() >= ExtraConfigs.server().kinetics.maxChainConveyorConnections.get())
		...
		if (!player.isCreative()) {
			...
			if (simulate)
				BlueprintOverlayRendererExtras.displayChainRequirements(chain.getItem(), chainCost, hasEnough);
			if (!hasEnough)
		...
		Lang.text("") // Clear status message
			.sendStatus(player);
		firstPos = null;
		firstDim = null;
		return true;
