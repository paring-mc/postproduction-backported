...
public class ValueSettingsScreen2 extends AbstractSimiScreen {

    private int ticksOpen;
    private ValueSettingsBoard2 board;
    private int maxLabelWidth;
    private int valueBarWidth;
    private BlockPos pos;
    private ValueSettingsBehaviour2.ValueSettings2 initialSettings;
    private ValueSettingsBehaviour2.ValueSettings2 lastHovered = new ValueSettingsBehaviour2.ValueSettings2(-1, -1);
    private Consumer<ValueSettingsBehaviour2.ValueSettings2> onHover;
    private boolean iconMode;
    private int milestoneSize;
    private int soundCoolDown;
    private int netId;

    public ValueSettingsScreen2(BlockPos pos, ValueSettingsBoard2 board, ValueSettingsBehaviour2.ValueSettings2 valueSettings,
                                Consumer<ValueSettingsBehaviour2.ValueSettings2> onHover, int netId) {
        this.pos = pos;
        this.board = board;
        this.initialSettings = valueSettings;
        this.onHover = onHover;
        this.netId = netId;
        this.iconMode = board.formatter() instanceof ValueSettingsFormatter2.ScrollOptionSettingsFormatter;
    ...
    public ValueSettingsBehaviour2.ValueSettings2 getClosestCoordinate(int mouseX, int mouseY) {
        int row = 0;
        int column = 0;
        ...
        return new ValueSettingsBehaviour2.ValueSettings2(row,
    ...
    protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
        ...
        Component tip =
                Lang.translateDirect("gui.value_settings.release_to_confirm", Components.keybind("key.use"));
        ...
        if (iconMode)
            for (int i = 0; i <= board.maxValue(); i++)
                fattestLabel = Math.max(fattestLabel, font.width(board.formatter()
                        .format(new ValueSettingsBehaviour2.ValueSettings2(0, i))));
            ...
            if (!iconMode) {
                ...
                for (int w = 0; w < valueBarWidth; w += AllGuiTextures.VALUE_SETTINGS_BAR.width - 1)
                    UIRenderHelper.drawCropped(graphics, valueBarX + w, y + 1,
                            Math.min(AllGuiTextures.VALUE_SETTINGS_BAR.width - 1, valueBarWidth - w), 8, zLevel,
        ...
        ValueSettingsBehaviour2.ValueSettings2 closest = getClosestCoordinate(mouseX, mouseY);
        ...
        if (board.formatter() instanceof ValueSettingsFormatter2.ScrollOptionSettingsFormatter sosf)
    ...
    public boolean mouseScrolled(double pMouseX, double pMouseY, double pDelta) {
        ValueSettingsBehaviour2.ValueSettings2 closest = getClosestCoordinate((int) pMouseX, (int) pMouseY);
    ...
    protected void saveAndClose(double pMouseX, double pMouseY) {
        ...
        ExtraPackets.getChannel()
                .sendToServer(new ValueSettingsPacket2(pos, closest.row(), closest.value(), null, null, Direction.UP,
