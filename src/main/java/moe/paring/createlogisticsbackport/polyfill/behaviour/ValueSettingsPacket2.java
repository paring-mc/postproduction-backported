...
public class ValueSettingsPacket2 extends BlockEntityConfigurationPacket<SmartBlockEntity> {
    private int row;
    private int value;
    ...
    public ValueSettingsPacket2(BlockPos pos, int row, int value, @Nullable InteractionHand interactHand,
    ...
    protected void applySettings(ServerPlayer player, SmartBlockEntity be) {
        for (BlockEntityBehaviour behaviour : be.getAllBehaviours()) {
            ...
            valueSettingsBehaviour.setValueSettings(player, new ValueSettingsBehaviour2.ValueSettings2(row, value), ctrlDown);
            return;
        }
    }
